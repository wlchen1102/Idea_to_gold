...
## 交互和用户体验：
- 前端页面实现时需要考虑鼠标悬浮、点击效果，遵循“先响应，后处理”原则，以用户体验为先。
- “先响应，后处理”原则：指的是用户触发事件后，立即响应事件，而不是等待事件处理完成后再响应。
- 鼠标悬浮效果：鼠标悬浮在元素上时，元素会有一定的变化，例如改变颜色、大小、位置等。
...
#### **规则三：数据库查询优化**
- 所有数据库查询都必须使用索引来加速查询速度。
- 避免使用`SELECT *`，而应明确指定需要的字段。
- 后端接口获取环境变量，在开发环境下，从`idea-to-gold\src\lib\env.ts`文件获取，在生产环境下，从Cloudflare Pages的环境变量获取。
...
#### **规则七：前端组件说明**
*   **【必须】** 所有前端组件都必须定义在 `src/components/` 目录下。当你想要新增组件的时候，必须要看看这个目录下是否已经存在了这个组件，避免重复定义。
*   **【必须】** 组件文件名必须使用 驼峰 命名规范。
*   **【必须】** 组件文件顶部必须包含一个注释，描述组件的功能和使用方法。
